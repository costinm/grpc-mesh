...
// period.
const (
	// PingServerPingProcedure is the fully-qualified name of the PingServer's Ping RPC.
	PingServerPingProcedure = "/fgrpc.PingServer/Ping"
)
...
func NewPingServerClient(httpClient connect_go.HTTPClient, baseURL string, opts ...connect_go.ClientOption) PingServerClient {
	...
	return &pingServerClient{
		ping: connect_go.NewClient[fgrpc.PingMessage, fgrpc.PingMessage](
			httpClient,
			baseURL+PingServerPingProcedure,
			opts...,
		),
	}
}
...
func NewPingServerHandler(svc PingServerHandler, opts ...connect_go.HandlerOption) (string, http.Handler) {
	pingServerPingHandler := connect_go.NewUnaryHandler(
		PingServerPingProcedure,
		svc.Ping,
		opts...,
	)
	return "/fgrpc.PingServer/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case PingServerPingProcedure:
			pingServerPingHandler.ServeHTTP(w, r)
		default:
			http.NotFound(w, r)
		}
	})
