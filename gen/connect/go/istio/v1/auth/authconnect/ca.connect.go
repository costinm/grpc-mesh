...
// period.
const (
	// IstioCertificateServiceCreateCertificateProcedure is the fully-qualified name of the
	// IstioCertificateService's CreateCertificate RPC.
	IstioCertificateServiceCreateCertificateProcedure = "/istio.v1.auth.IstioCertificateService/CreateCertificate"
)
...
func NewIstioCertificateServiceClient(httpClient connect_go.HTTPClient, baseURL string, opts ...connect_go.ClientOption) IstioCertificateServiceClient {
	...
	return &istioCertificateServiceClient{
		createCertificate: connect_go.NewClient[auth.IstioCertificateRequest, auth.IstioCertificateResponse](
			httpClient,
			baseURL+IstioCertificateServiceCreateCertificateProcedure,
			opts...,
		),
	}
}
...
func NewIstioCertificateServiceHandler(svc IstioCertificateServiceHandler, opts ...connect_go.HandlerOption) (string, http.Handler) {
	istioCertificateServiceCreateCertificateHandler := connect_go.NewUnaryHandler(
		IstioCertificateServiceCreateCertificateProcedure,
		svc.CreateCertificate,
		opts...,
	)
	return "/istio.v1.auth.IstioCertificateService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case IstioCertificateServiceCreateCertificateProcedure:
			istioCertificateServiceCreateCertificateHandler.ServeHTTP(w, r)
		default:
			http.NotFound(w, r)
		}
	})
